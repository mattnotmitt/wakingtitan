...
const checkSites = async(bot) => {
	...
	for (let site in data.wtSites.sites) {
		...
		request({
			...
			const oldCont = jetpack.read(`/home/matt/mattBot/watcherData/${data.wtSites.sites[site]}-latest.html`);
					...
					request({
						...
						if (pageCont2 === pageCont) {
							if (!hasUpdate[site]) {
								...
								jetpack.write(`/home/matt/mattBot/watcherData/${data.wtSites.sites[site]}-temp.html`, pageCont);
								exec(`diffchecker /home/matt/mattBot/watcherData/${data.wtSites.sites[site]}-latest.html /home/matt/mattBot/watcherData/${data.wtSites.sites[site]}-temp.html`).then(async res => {
									let status;
									...
									if (site === 'https://wakingtitan.com') checkGlyphs(bot);
									//T.post('statuses/update', {
									//	status: status
									//}).catch(err => bot.error(exports.data.name, err));
									for (let channel of data.wtSites.channels) {
										bot.channels.get(channel).send('', {
											embed: embed
										});
									}
									await request(`https://web.archive.org/save/${site}`);
									jetpack.remove(`/home/matt/mattBot/watcherData/${data.wtSites.sites[site]}-temp.html`);
									jetpack.write(`/home/matt/mattBot/watcherData/${data.wtSites.sites[site]}-latest.html`, pageCont);
									jetpack.write(`/home/matt/mattBot/watcherData/${data.wtSites.sites[site]}-logs/${strftime('%F - %H-%M-%S')}.html`, pageCont);
									hasUpdate[site] = true;
								});
							}
						} else {
...
const checkGlyphs = (bot) => {
	request({
		...
		for (let i = 0; i < glyphs.length; i++) {
			if (glyphs.sort()[i] !== data.wtSites.glyphs[i]) {
		...
		if (change) {
			data.wtSites.glyphs = glyphs.sort();
